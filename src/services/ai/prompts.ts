...
// All system prompt construction logic — fully typed, per-mode, professional
// ─────────────────────────────────────────────────────────────────────────────

import { getCitationFormat, type CitationFormat } from "./citationFormats";
import type { AiMode, MindsetType, DepthLevel } from "./types";

// ─────────────────────────────────────────────────────────────────────────────
// Depth guides
// ─────────────────────────────────────────────────────────────────────────────

const DEPTH_GUIDES: Record<DepthLevel, string> = {
  beginner: `
DEPTH: Beginner
...
function depthSection(depth?: DepthLevel): string {
...
const MINDSET_GUIDES: Record<MindsetType, string> = {
  general:
...
function mindsetSection(mindset?: MindsetType): string {
...
function buildTutorPrompt(mindset?: MindsetType, depth?: DepthLevel): string {
  return `
...
function buildResearchPrompt(mindset?: MindsetType, depth?: DepthLevel): string {
  return `
 ...
 * --------------------
 * This prompt does TWO things simultaneously:
 *   1. Researches the user's TOPIC deeply (like buildResearchPrompt)
 *   2. Presents that research formatted as a proper academic paper in the
 *      chosen citation style.
 *
 * The previous version only explained citation formatting rules and ignored
 * the topic entirely — producing a style guide instead of a research report.
 * This version fixes that by making the topic research the primary goal and
 * treating citation formatting as the output presentation layer.
 */
function buildCitationResearchPrompt(
  format: CitationFormat,
  mindset?: MindsetType,
  depth?: DepthLevel
): string {
  // ── Infer paper structure section names from the format ──────────────────
  const paperSections: string[] = format.structure?.sections
...
/**
 * Build the full system prompt for a given AI mode and options.
 * This is the single entry point used by aiService.ts.
 */
